...
# 获得按平均法获得下车人数
def get_alight_num(test_dt='20141229'):
    ...
    alight_df = alight_df[(pd.to_datetime(alight_df.in_time) >= star_time) & (pd.to_datetime(alight_df.in_time) <= end_time)
                & (alight_df.in_station > 14) & (alight_df.out_station == 14)].loc[:, ['in_time', 'total_time']]
    time_table = pd.read_csv(
        'E:\Pycharm\PythonProjects\Subway\data\TrainData\TrainData_for14_line1_' + test_dt + '.csv')#.loc[:, 'leav_time']
    time_table.leav_time *= 15
    out_seconds = []
    ...
    for i in range(1, 36):
        check_seconds = i * 1800
        alight_total = alight_df[(alight_df.out_seconds > check_seconds - 1800)
                                 & (alight_df.out_seconds <= check_seconds)].shape[0]
        train_total = time_table[(time_table.leav_time > check_seconds - 1800)
                                 & (time_table.leav_time <= check_seconds)].shape[0]
        if train_total != 0:
            temp = alight_total/train_total
            for j in range(train_total):
                alight_nums.append(temp)
    return alight_nums


# 获得对比算法的预测结果
def get_avg_alight_predict(test_dt='20141229'):
    C = 1880
    alight_proportion = 0.04#get_alight_proportion()
    star_time = datetime.datetime.strptime(test_dt + '063000', '%Y%m%d%H%M%S')
    end_time = datetime.datetime.strptime(test_dt + '233000', '%Y%m%d%H%M%S')
