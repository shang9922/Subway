...
# 基于短时时刻表的预测
def short_term_predict(dts, svr_dt, test_dt):
    predict_time_table = STP.predict_by_constant_weights(dts, test_dt)  # 获得预测的时刻表，从第5班车开始，长度200
    ...
    term_num = math.ceil(float(len(predict_time_table))/3)  # 预测的轮数
    ...
    for i in range(int(term_num)):                       # 每轮
        ...
        del pre_tab[:]
        del pre_rem[:]
        most_train = min((i + 1) * 3 + 4, real_time_table.shape[0])     # 本轮预测到第几班车
        pice_num = (real_time_table[most_train - 1] - last_remain_sec)/15 + 1   # 本轮总共预测的时间片数
        for p in range(i * 3, most_train - 4):
...
# plt.plot(test.check_seconds[200:800], ha['count'][200:800], c='r')
# plt.show()
# print math.sqrt(mean_squared_error(test['count'], ha['count']))
# print mean_absolute_error(test['count'], ha['count'])


'''基于真实时刻表的预测'''
# avg_time_table = pd.read_csv('E:\Pycharm\PythonProjects\Subway\data\TrainData\TrainData_for14_line1_20141229.csv').loc[:, 'leav_time']
# avg_time_table *= 15
# Arima_rs = AP.arimaMainProcess(dts, 204, test_dt)
# flow = decompose_flow(Arima_rs)     # 存放预测得到的流量，以15s为单位，没有进过取整处理
# svr_model = get_SVR_model(get_svr_train(svr_dt))    # 训练好的SVR模型
# walk_dua = 3            # 步行流量延迟
# predict_rs = []         # 存放预测的结果
...
#                            columns=['train_num', 'time_pice', 'day_type', 'duration', 'leav_time', 'total_num',
#                                     'pre_1', 'pre_2', 'pre_3']).values
#     Y_pre = predict_Board_num(get_svr_train(svr_dt), svr_model, X_test)
...
#
# test = pd.read_csv('E:\Pycharm\PythonProjects\Subway\data\PlatformCount\PlatformCount_for_' + test_dt + '.csv').loc[:, ['check_seconds', 'count']]
# # plt.plot(test.check_seconds[0:600], test['count'][0:600], c='b')
# # plt.plot(test.check_seconds[0:600], predict_rs[0:600], c='r')
# # plt.show()
# # plt.plot(test.check_seconds[600:1200], test['count'][600:1200], c='b')
# # plt.plot(test.check_seconds[600:1200], predict_rs[600:1200], c='r')
# # plt.show()
# # plt.plot(test.check_seconds[1200:1800], test['count'][1200:1800], c='b')
# # plt.plot(test.check_seconds[1200:1800], predict_rs[1200:1800], c='r')
# # plt.show()
# # plt.plot(test.check_seconds[1800:2400], test['count'][1800:2400], c='b')
# # plt.plot(test.check_seconds[1800:2400], predict_rs[1800:2400], c='r')
# # plt.show()
# # plt.plot(test.check_seconds[3600:4200], test['count'][3600:4200], c='b')
# # plt.plot(test.check_seconds[3600:4200], predict_rs[3600:4200], c='r')
# # plt.show()
# print math.sqrt(mean_squared_error(test['count'], predict_rs))
# print mean_absolute_error(test['count'], predict_rs)

predict_rs = short_term_predict(dts, svr_dt, test_dt)
real_train_data = pd.read_csv('E:\Pycharm\PythonProjects\Subway\data\TrainData\TrainData_for14_line1_' + test_dt + '.csv')
real_count = pd.read_csv(
...
# plt.plot(test.check_seconds[0:600], real_data['count'][0:600], c='b')
# plt.plot(test.check_seconds[0:600], predict_rs[0:600], c='r')
# plt.show()
