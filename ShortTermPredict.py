...
def get_avg_duration(start, end, dts):
    ...
    for date_str in dts:                # 分别计算每天的间隔时间
        ...
        if start < temp_df.shape[0] and end <= temp_df.shape[0]:
            derta = temp_df.iloc[end-1, 1] - temp_df.iloc[start-1, 1]
    ...
    for date_str in dts:
        ...
        if k < temp_df.shape[0]:
            for i in range(temp_df.shape[0] - k):
                start_seconds = temp_df.iloc[i, 1]
                train.append(K_predict(i+1, start_seconds, k, df))
                test.append(temp_df.iloc[i+k, 1])
        else:
            return -1
...
def get_mean_squared_error_matrix(dts):
    ...
    rs_df.to_csv('E:\Pycharm\PythonProjects\Subway\data\TimeTable\mean_squared_error.csv')
    plt.plot(X, Y)
    plt.show()
...
def output_avg_du_matric_to_CSV():
    ...
    avg_du_df.to_csv('E:\Pycharm\PythonProjects\Subway\data\TimeTable\Avg_Duration_Matric.csv')


# 计算历史平均时刻表在训练集上的预测均方差
def get_HA_mean_squared_error(dts):
    ha_seconds = pd.read_csv('E:\Pycharm\PythonProjects\Subway\data\TimeTable\Avg_Time_Table.csv').check_seconds
    temp = []
    for dt in dts:
        test_seconds = pd.read_csv(
            'E:\Pycharm\PythonProjects\Subway\data\TimeTable\TimeTable_for14_line1_' + dt + '.csv').check_seconds
        temp.append(mean_squared_error(test_seconds, ha_seconds))
    return np.mean(temp)

# 固定权重下的预测
def predict_by_constant_weights(dts, test_str):
    ha_seconds = list(pd.read_csv( 'E:\Pycharm\PythonProjects\Subway\data\TimeTable\Avg_Time_Table.csv').iloc[:, 2])
    test_df = pd.read_csv('E:\Pycharm\PythonProjects\Subway\data\TimeTable\TimeTable_for14_line1_' + test_str + '.csv')
    test_seconds = list(test_df.check_seconds)
    avg_du_matric = pd.read_csv(
        'E:\Pycharm\PythonProjects\Subway\data\TimeTable\Avg_Duration_Matric.csv').iloc[:, 1:7].values
    fangcha_ha = get_HA_mean_squared_error(dts)
    fangcha_du = list(pd.read_csv(
        'E:\Pycharm\PythonProjects\Subway\data\TimeTable\mean_squared_error.csv').iloc[0:6, 1].values)
